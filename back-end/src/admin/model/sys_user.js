...
module.exports = class extends think.Model {
	async addData(param) {
		...
		param.password = think.md5(param.password);
		param.status = 1;
		// 如果没有提供电话号码，使用空字符串
		if (!param.phone) {
			param.phone = '';
	...
	async login(param) {
		let data = await this.alias('a').join({
			...
			as: 'b',
			on: ['role_id', 'id']
		}).field("a.*, b.name as rolename").where({ login_name: param.login_name, password: think.md5(param.password) }).find();
		if (!think.isEmpty(data) && data.id == '1') {
			data.rolename = '超级管理员';
		}
		return data;
	}
